...
function run (args) {
    var arg, next, watch, ignore, pidFilePath, program, extensions, executor, poll_interval, debugFlag, debugBrkFlag, debugBrkFlagArg, harmony, inspect, exitGracefully;
    ...
    try {
        ...
        [ "SIGTERM", "SIGINT", "SIGHUP", "SIGQUIT" ].forEach( function(signal) {
            process.on(signal, function () {
                ...
                if (pidFilePath){
                    deletePidFile();
                } else if (!(signal === "SIGINT" && exitGracefully)) {
                    process.exit();
                }
            });
        });
    ...
    startChildProcess = function() { startProgram(program, executor, exitGracefully); };
...
function startProgram (prog, exec, exitGracefully) {
    ...
    child.addListener("exit", function (code) {
        ...
        if (!crash_queued) {
            ...
            exports.child = null;
            if (noRestartOn == "exit" || noRestartOn == "error" && code !== 0 || noRestartOn == "success" && code === 0) {
                if (exitGracefully) {
                    process.exit();
                } else {
                    return;
